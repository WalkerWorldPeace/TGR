...
def pretrain(args,specific,device):
    ...
    for epoch in range(num_epoch):
        ...
        if best_acc==None or best_acc<test_acc:
            ...
            if not_increase==60:#7 for cifar and mini;
...
def average_cosine_similarity(grads):
    num_grads = len(grads)
    total_similarity = 0
    count = 0
    for i in range(num_grads):
        for j in range(i+1, num_grads):
            grad1_flattened = torch.cat([g.view(-1) for g in grads[i] if g is not None])
            grad2_flattened = torch.cat([g.view(-1) for g in grads[j] if g is not None])
            # compute cosine similarity
            similarity = F.cosine_similarity(grad1_flattened, grad2_flattened, dim=0)
