...
class ImagePool2(object):
    def __init__(self,args, root,num_classes,transform,max_batch_per_class):
        ...
        self.root_pre = os.path.join('./pretrained_model/{}/{}'.format(args.dataset, args.pre_backbone), 'inversion')
        self.root_abl = os.path.join('./pretrained_model/{}/{}'.format(args.dataset, args.pre_backbone), 'abl')
        self._idx = [0] * num_classes
        self._idx_pre = [[0] * num_classes for _ in range(100)]
    ...
    def add_pre(self, imgs, c_abs_list=None, node_id=None, targets=None):
        label_list = list(range(len(c_abs_list)))
        for label in label_list:
            root = os.path.join(self.root_pre, str(node_id), str(label))
            os.makedirs(root, exist_ok=True)
            imgs_c = imgs[targets == label]
            save_image_batch(imgs_c, os.path.join(root, "%d.png" % (self._idx_pre[node_id][label])), pack=False)
            self._idx_pre[node_id][label] += 1

    def add_abl(self, imgs, c_abs_list=None, node_id=None, targets=None):
        label_list = list(range(len(c_abs_list)))
        for label in label_list:
            root = os.path.join(self.root_abl, str(node_id), str(label))
            os.makedirs(root, exist_ok=True)
            imgs_c = imgs[targets == label]
            save_image_batch(imgs_c, os.path.join(root, "%d.png" % (self._idx_pre[node_id][label])), pack=False)
            self._idx_pre[node_id][label] += 1
